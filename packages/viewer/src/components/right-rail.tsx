...
import { AnnotationFooterHostActions } from '@/components/host-actions';
import { glass } from '@/components/glass';
import { cn } from '@/lib/utils';
import type { Annotation } from '@/marks/types';
import type { RenderMode } from '@/scene/viewer';
import { useAnnotations, useViewerState, useViewerStore, type MarkMode } from '@/store';
 ...
 */
export function RightRail({ autoOpenAnnotations = true }: { autoOpenAnnotations?: boolean }) {
  ...
  useEffect(() => {
    if (autoOpenAnnotations && count === 1) {
      setMarksOpen(true);
    }
  }, [autoOpenAnnotations, count]);
        ...
        >
          ...
          {count === 0 ? (
            ...
            <ul className="flex-1 overflow-y-auto p-2">
              {annotations.map(a => (
                <MarkRow key={a.id} annotation={a} onRemove={() => marksRuntime.removeAnnotation(a.id)} />
              ))}
            </ul>
          )}
          <AnnotationFooterHostActions />
        </aside>
      )}
    </div>
  );
